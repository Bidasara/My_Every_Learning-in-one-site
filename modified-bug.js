...
document.getElementById('bugs-btn').addEventListener('click', function() {

    if(!activated)
    {
        // Create flies
        window.myBugs = new BugController({
            minBugs: 5,
            maxBugs: 10
        });
        
        // Create spiders
        window.mySpiders = new SpiderController({
            minBugs: 2,
            maxBugs: 5
        });
        document.getElementById("display_content5").innerHTML="Hahhh, you brought this on yourself, Now press again for back to normal"
        activated=true;
    }
    else
    {
        if (window.myBugs) window.myBugs.end();
        if (window.mySpiders) window.mySpiders.end();
        document.getElementById("display_content5").innerHTML="Don't Press this button"
        activated=false;
    }
});